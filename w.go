...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_481(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_406(st, Dm_build_681)
	if err != nil {
		return err
	}
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_683)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_682)
	if err != nil {
		return nil, err
	}
...
func encodeArgs(stmt *DmStatement, args []driver.Value) ([]interface{}, error) {
	...
	nextSwitch:
		if arg == nil {
			if stmt.params[i].colType == CURSOR && stmt.params[i].cursorStmt == nil {
				...
				err = stmt.params[i].cursorStmt.dmConn.Access.Dm_build_388(stmt.params[i].cursorStmt)
			} else if resetColType(stmt, i, NULL) {
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_438(stmt, bytes, false)
	if err != nil {
		return err
	}
			...
			if stmt.curRowBindIndicator[i]&BIND_OUT == BIND_OUT {
				if outParamData == nil {
					if arg, ok := args[i].(*driver.Rows); ok && stmt.params[i].colType == CURSOR {
						...
						if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_448(stmt.params[i].cursorStmt, 1); err != nil {
							return err
						}
				...
				nextSwitch:
					...
					case *driver.Rows:
						if stmt.params[i].colType == CURSOR {
							var tmpExecInfo *execInfo
							tmpExecInfo, err = stmt.dmConn.Access.Dm_build_448(stmt.params[i].cursorStmt, 1)
							if err != nil {
								return err
							}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_427(stmt, bytes)
	return err
}
