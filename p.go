...
const (
	...
	compressDef   = Dm_build_677
	compressIDDef = Dm_build_678
...
func (c *DmConnector) setAttributes(props *Properties) error {
	...
	c.bufPrefetch = props.GetInt(BufPrefetchKey, c.bufPrefetch, int(Dm_build_662), int(Dm_build_663))
...
func (c *DmConnector) parseOsAuthType(props *Properties) error {
	...
	if value != "" && !util.StringUtil.IsDigit(value) {
		if util.StringUtil.EqualsIgnoreCase(value, "ON") {
			...
			c.osAuthType = Dm_build_655
		}
	} else {
		c.osAuthType = byte(props.GetInt(OsAuthTypeKey, int(c.osAuthType), 0, 4))
	}
	if c.user == "" && c.osAuthType == Dm_build_655 {
		c.user = "SYSDBA"
	} else if c.osAuthType != Dm_build_655 && c.user != "" {
		ECGO_OSAUTH_ERROR.throw()
	} else if c.osAuthType != Dm_build_655 {
